...
package body Lumen.Image is
   ...
   function From_File (Pathname : String) return Descriptor is
      ...
      FITS_Signature : constant Byte_String := (16#53#, 16#49#, 16#4D#,
                                                16#50#, 16#4C#, 16#45#);
      JFIF_Signature : constant Byte_String := (16#FF#, 16#D8#);
      PNG_Signature  : constant Byte_String := (16#89#, 16#50#, 16#4E#, 16#47#,
   ...
   begin  -- From_File
      ...
      if Sig (Sig'First .. Sig'First + PNG_Signature'Length - 1) =
         PNG_Signature
      then
         Binary.IO.Close (File);
         -- Until we start to support PNG
         raise Unknown_Format with "PNG image loading not yet supported";
      elsif Sig (Sig'First .. Sig'First + JFIF_Signature'Length - 1) =
            JFIF_Signature
      then
         Binary.IO.Close (File);
         -- until we start to support JPEG/JFIF
         raise Unknown_Format with "JPEG image loading not yet supported";
      elsif Sig (Sig'First .. Sig'First + FITS_Signature'Length - 1) =
            FITS_Signature
      then
         Binary.IO.Close (File);
         -- until we start to support FITS
         raise Unknown_Format with "FITS image loading not yet supported";
      elsif Sig (Sig'First .. Sig'First + BMP_Signature'Length - 1) =
            BMP_Signature
      then
         Result := BMP.From_File (File);
         Binary.IO.Close (File);
         return Result;
      elsif Sig (Sig'First) = 16#50# and
            Is_In (Sig (Sig'First + 2), Whitespace)
      then  -- "P"
         PPM_Sub := Character'Val (Sig (Sig'First + 1));
         if PPM_Sub in '1' .. '6' then
            Result := PPM.From_File (File, PPM_Sub);
            Binary.IO.Close (File);
            return Result;
         else
            Binary.IO.Close (File);
            -- something else that starts with "P"
            raise Unknown_Format with "unsupported PPM or other unknown format";
         end if;
      else
         Binary.IO.Close (File);
         -- some format we don't recognize
         raise Unknown_Format with "cannot recognize image format";
      end if;
